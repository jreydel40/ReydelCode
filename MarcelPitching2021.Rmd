...
pitchingstats2017$SLG <- pitchingstats2017$SLG * 0.95

pitchingstats2017$ERA <- pitchingstats2017$ERA * 0.95

pitchingstats2017$FIPAdjMinus <- pitchingstats2017$FIPAdjMinus * 0.95

pitching_stats_2020_weighted <- pitchingstats2020_selected[, numeric_columns] * 5
pitching_stats_2019_weighted <- pitchingstats2019_selected[, numeric_columns] * 4
pitching_stats_2018_weighted <- pitchingstats2018_selected[, numeric_columns] * 3
```
...
```{r}
combined_stats <- left_join(pitching_stats_2018_weighted, pitching_stats_2019_weighted, by = "PlayerID", suffix = c("_2018", "_2019"))

combined_stats <- left_join(combined_stats, pitching_stats_2020_weighted, by = "PlayerID", suffix = c("", "_2020"))
...
combined_stats_weighted <- combined_stats_unique %>%
  mutate(weighted_IP = (IP_2018 + IP_2019 + IP) / 12) %>%
  mutate(weighted_PA = (PA_2018 + PA_2019 + PA) / 12) %>%
  mutate(weighted_BBRate = (BBRate_2018 + BBRate_2019 + BBRate) / 12) %>%
  mutate(weighted_SORate = (SORate_2018 + SORate_2019 + SORate) / 12) %>%
  mutate(weighted_ERA = (ERA_2018 + ERA_2019 + ERA) / 12) %>%
  ...
  mutate(weighted_FIPAdjMinus = (FIPAdjMinus_2018 + FIPAdjMinus_2019 + FIPAdjMinus) / 12)
#%>%
 # mutate(weighted_wRAA = (wRAA_2018 + wRAA_2019 + wRAA) / 12)
```
...
```{r}
merged_stats_w_2021 <- left_join(pitchingstats2021_selected, weighted_stats, by = c("PlayerID", "Name"))
...
```{r}
age_adj_stats_w_2021_unique <- age_adj_stats_w_2021 %>%
  distinct(Name, .keep_all = TRUE)
model_performance_2021 <- age_adj_stats_w_2021_unique %>%
  summarise(sum_perc_diff = sum(total_perc_diff)/nrow(age_adj_stats_w_2021_unique))
model_performance_2021
```

```{r}
# Linear regression models
model_avg <- lm(AVG ~ proj_AVG, data = age_adj_stats_w_2021_unique)
model_obp <- lm(OBP ~ proj_OBP, data = age_adj_stats_w_2021_unique)
model_slg <- lm(SLG ~ proj_SLG, data = age_adj_stats_w_2021_unique)
...
ggplot(age_adj_stats_w_2021_unique, aes(x = proj_AVG, y = AVG)) +
...
ggplot(age_adj_stats_w_2021_unique, aes(x = proj_OBP, y = OBP)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
...
ggplot(age_adj_stats_w_2021_unique, aes(x = proj_SLG, y = SLG)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
...
```{r}
# Linear regression models
model_era <- lm(ERA ~ proj_ERA, data = age_adj_stats_w_2021_unique)
model_fip <- lm(FIPAdjMinus ~ proj_FIPAdjMinus, data = age_adj_stats_w_2021_unique)
model_so <- lm(SORate ~ proj_SORate, data = age_adj_stats_w_2021_unique)
...
ggplot(age_adj_stats_w_2021_unique, aes(x = proj_ERA, y = ERA)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
...
ggplot(age_adj_stats_w_2021_unique, aes(x = proj_FIPAdjMinus, y = FIPAdjMinus)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
...
ggplot(age_adj_stats_w_2021_unique, aes(x = proj_SORate, y = SORate)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", se = FALSE, color = "black") +
