...
```{r}
logregct <- glmer(IsStrike ~  #Acceleration in X direction
                        ...
                        y0 + #Starting point in the Y direction
                         #Starting point in the Z direction
                        ...
                        vz0 + #Velocity in the Z direction
                         #IT: velocity in the Z firection with platetime
                         #Absolute value of pxNew
                        release:velo + #IT: total release vector with total velocity vector
                         #Bottom of strike zone for specific hitter 
                         #IT: CenteredX with velocity in the X direction
                        (1 | F2) + #CatcherID random effect
                        (1 | PitcherId) + #Pitcher ID random effect
                        (1 | UmpHomeId)  #Umpire ID random effect
                        , #IT: StrikeZoneBottom with velocity in the Z direction
                        data = train, family = "binomial", , control = glmerControl(optimizer = "bobyqa"))
summary(logregct)
```
```{r}
save(logregct, file = "logreg_RR_FC.RData")
...
logregtest <- glmer(IsStrike ~ ax +
                        ...
                        #x0:vx0 +
                        #y0:vy0 +
                        #z0:vz0 + #this changed the visual drastically
                        #x0:accel +
                        #y0:accel +
                        #z0:accel +
                        #x0:velo +
                       ...
                       # z0:veloV +
                        ...
                        (1 | UmpHomeId) +
                        (1 | BatterId) +
                        StrikeZoneBottom:vz0, data = train, family = "binomial", , control = glmerControl(optimizer = "bobyqa"))
summary(logregtest)
```


```{r}
logreg_preds1 <- predict(logregct, test, type = "response", re.form = NA) #StrikePerc
