...
mlb_stats_2021_df <- mlb_stats_2021_df %>%
  mutate(BBRate = BB/ PA) %>%
  mutate(KRate = SO/ PA)
```
...
mlb_stats_2020_weighted <- mlb_stats_2020_selected[, numeric_columns] * 5
mlb_stats_2019_weighted <- mlb_stats_2019_selected[, numeric_columns] * 4
mlb_stats_2018_weighted <- mlb_stats_2018_selected[, numeric_columns] * 3
```
...
```{r}
combined_stats <- left_join(mlb_stats_2018_weighted, mlb_stats_2019_weighted, by = "PlayerID", suffix = c("_2018", "_2019"))

combined_stats <- left_join(combined_stats, mlb_stats_2020_weighted, by = "PlayerID", suffix = c("", "_2020"))
combined_stats_filter <- combined_stats %>%
  filter(!is.na(PA_2018) & !is.na(PA_2019) & !is.na(PA))
combined_stats_unique <- combined_stats_filter %>%
  distinct(Name, .keep_all = TRUE)
```
```{r}
combined_stats_weighted <- combined_stats_unique %>%
  mutate(weighted_PA = (PA_2018 + PA_2019 + PA) / 12) %>%
  mutate(weighted_G = (G_2018 + G_2019 + G) / 12) %>%
  mutate(weighted_AB = (AB_2018 + AB_2019 + AB) / 12) %>%
  mutate(weighted_H = (H_2018 + H_2019 + H) / 12) %>%
  mutate(weighted_B1 = (B1_2018 + B1_2019 + B1) / 12) %>%
  mutate(weighted_B2 = (B2_2018 + B2_2019 + B2) / 12) %>%
  mutate(weighted_B3 = (B3_2018 + B3_2019 + B3) / 12) %>%
  mutate(weighted_HR = (HR_2018 + HR_2019 + HR) / 12) %>%
  mutate(weighted_BB = (BB_2018 + BB_2019 + BB) / 12) %>%
  mutate(weighted_SO = (SO_2018 + SO_2019 + SO) / 12) %>%
  mutate(weighted_AVG = (AVG_2018 + AVG_2019 + AVG) / 12) %>%
  mutate(weighted_OBP = (OBP_2018 + OBP_2019 + OBP) / 12) %>%
  mutate(weighted_SLG = (SLG_2018 + SLG_2019 + SLG) / 12) %>%
  mutate(weighted_TB = (TB_2018 + TB_2019 + TB) / 12) %>%
  mutate(weighted_HBP = (HBP_2018 + HBP_2019 + HBP) / 12) %>%
  mutate(weighted_SH = (SH_2018 + SH_2019 + SH) / 12) %>%
  mutate(weighted_SF = (SF_2018 + SF_2019 + SF) / 12) %>%
  mutate(weighted_BBRate = (BBRate_2018 + BBRate_2019 + BBRate) / 12) %>%
  mutate(weighted_KRate = (KRate_2018 + KRate_2019 + KRate) / 12) 
```
...
weighted_stats <- combined_stats_weighted %>%
  ...
  filter(weighted_PA >= 300)
```

```{r}
merged_stats_w_2021 <- left_join(mlb_stats_2021_df, weighted_stats, by = c("PlayerID", "Name"))
merged_stats_w_2021 <- merged_stats_w_2021 %>%
...
```{r}
age_adj_stats_w_2021 <- merged_stats_w_2021 %>%
...
age_adj_stats_w_2021 <- age_adj_stats_w_2021 %>%
...
```{r}
selected_stats <- age_adj_stats_w_2021 %>%
...
selected_stats_2021 <- age_adj_stats_w_2021 %>%
...
print(selected_stats_2021)

model_performance_2021 <- selected_stats_2021 %>%
  summarise(sum_perc_diff = sum(total_perc_diff)/nrow(selected_stats_2021))
model_performance_2021
```
...
query2021 <- "SELECT [Name]
      ...
      ,[UpdatedDate]
  FROM [mlb_stats].[dbo].[stat_batter_basic] d
  WHERE d.Season = 2021 AND d.LeagueLevel = 1 and d.GameType" 
mlb_stats_2021 <- sqlQuery(db_intern_connection, query2021)
print(mlb_stats_2021)
...
```{r}
combined_stats <- left_join(mlb_stats_2021_weighted, mlb_stats_2019_weighted, by = "PlayerID", suffix = c("_2021", "_2019"))

combined_stats <- left_join(combined_stats, mlb_stats_2020_weighted, by = "PlayerID", suffix = c("", "_2020"))
combined_stats_filter <- combined_stats %>%
  filter(!is.na(PA_2021) & !is.na(PA_2019) & !is.na(PA))
combined_stats_unique <- combined_stats_filter %>%
  distinct(Name, .keep_all = TRUE)
```
```{r}
combined_stats_weighted <- combined_stats_unique %>%
  mutate(weighted_PA = (PA_2021 + PA_2019 + PA) / 12) %>%
  mutate(weighted_G = (G_2021 + G_2019 + G) / 12) %>%
  mutate(weighted_AB = (AB_2021 + AB_2019 + AB) / 12) %>%
  mutate(weighted_H = (H_2021 + H_2019 + H) / 12) %>%
  mutate(weighted_B1 = (B1_2021 + B1_2019 + B1) / 12) %>%
  mutate(weighted_B2 = (B2_2021 + B2_2019 + B2) / 12) %>%
  mutate(weighted_B3 = (B3_2021 + B3_2019 + B3) / 12) %>%
  mutate(weighted_HR = (HR_2021 + HR_2019 + HR) / 12) %>%
  mutate(weighted_BB = (BB_2021 + BB_2019 + BB) / 12) %>%
  mutate(weighted_SO = (SO_2021 + SO_2019 + SO) / 12) %>%
  mutate(weighted_AVG = (AVG_2021 + AVG_2019 + AVG) / 12) %>%
  mutate(weighted_OBP = (OBP_2021 + OBP_2019 + OBP) / 12) %>%
  mutate(weighted_SLG = (SLG_2021 + SLG_2019 + SLG) / 12) %>%
  mutate(weighted_TB = (TB_2021 + TB_2019 + TB) / 12) %>%
  mutate(weighted_HBP = (HBP_2021 + HBP_2019 + HBP) / 12) %>%
  mutate(weighted_SH = (SH_2021 + SH_2019 + SH) / 12) %>%
  mutate(weighted_SF = (SF_2021 + SF_2019 + SF) / 12) %>%
  mutate(weighted_BBRate = (BBRate_2021 + BBRate_2019 + BBRate) / 12) %>%
  mutate(weighted_KRate = (KRate_2021 + KRate_2019 + KRate) / 12) 
```
...
model_performance <- selected_stats %>%
  summarise(sum_perc_diff = sum(total_perc_diff)/nrow(selected_stats))
model_performance
```
